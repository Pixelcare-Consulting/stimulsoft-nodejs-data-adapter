  ...
  try {
    ...
    var onQuery = function (recordset, maxDataRows) {
      ...
      if (
        ...
        Array.isArray(recordset.rows[0])
      )
        recordset.rows = recordset.rows[0];
      ...
      for (var recordIndex in recordset.rows) {
        ...
        for (var columnName in recordset.rows[recordIndex]) {
          ...
          if (recordset.rows[recordIndex][columnName] instanceof Uint8Array) {
            ...
            recordset.rows[recordIndex][columnName] = Buffer.from(
              recordset.rows[recordIndex][columnName]
            ).toString('base64');
          }

          if (
            recordset.rows[recordIndex][columnName] != null &&
            typeof recordset.rows[recordIndex][columnName].toISOString ===
              'function'
          ) {
            if (types[columnIndex] == 'datetimeZ') {
              recordset.rows[recordIndex][columnName] =
                recordset.rows[recordIndex][columnName].toISOString();
            } else {
              var dateTime = new Date(
                recordset.rows[recordIndex][columnName].getTime() -
                  recordset.rows[recordIndex][columnName].getTimezoneOffset() *
                    60000
              ).toISOString();
              recordset.rows[recordIndex][columnName] = dateTime.replace(
                'Z',
                ''
              );
              types[columnIndex] = 'datetime';
          ...
          if (match && match.length >= 2) {
            ...
            switch (match[0]) {
              ...
              case 'ssl':
                ...
                break;
              case 'sslmode':
                if (match[1] == 'require' || match[1] == 'Require') {
                  info['ssl'] = 1;
    ...
    var applyQueryParameters = function (
      baseSqlCommand,
      baseParameters,
      ...
      if (baseSqlCommand != null && baseSqlCommand.indexOf('@') > -1) {
        while (
          ...
          baseSqlCommand = baseSqlCommand.substring(
            baseSqlCommand.indexOf('@') + 1
          );
          ...
          var parameter = baseParameters.find(
            (parameter) =>
          ...
          if (parameter) {
            if (parameter.index == null) {
              ...
              else if (parameter.typeGroup == 'datetime')
                parameters.push(new Date(parameter.value));
    ...
    if (command.connectionString.startsWith('postgres://'))
      ...
      command.postgreConnectionString =
        'postgres://' +
        command.connectionStringInfo.user +
        ':' +
      ...
      if (command.connectionStringInfo.port != null)
        command.postgreConnectionString +=
          ':' + command.connectionStringInfo.port;
      command.postgreConnectionString +=
        '/' +
